...
export const TableGrupoUsuarios = ({
  grupoUsuarios,
  search,
  selectedGrupoUsuario,
  ...
  const [totalGrupoUsuarios, setTotalGrupoUsuarios] = useState(
    JSON.parse(sessionStorage.getItem("totalGrupoUsuarios") ?? false)
  );
  const [itemsPerPage, setItemsPerPage] = useState(10);
  const [grupoUsuariosList, setGrupoUsuariosList] = useState([]);

  const openModal = (grupoUsuario) => {
    setShow(true);
    setSelectedGrupoUsuario(grupoUsuario);
  };

  const updateGrupoUsuariosList = () => {
    let updatedList = grupoUsuarios;
    if (!totalGrupoUsuarios) {
      updatedList = grupoUsuarios.filter((grupoUsuario) => grupoUsuario.Activo);
    }
    setGrupoUsuariosList(updatedList);
    goToPage(1);
  };

  const grupoUsuariosListFiltered = grupoUsuariosList?.filter((grupoUsuario) => {
    return Object.entries(grupoUsuario).some(([clave, valor]) => {
  ...
  const handleChangeTotalEspecialidades = () => {
    setTotalGrupoUsuarios((prevTotalGrupoUsuarios) => !prevTotalGrupoUsuarios);
    sessionStorage.setItem("totalGrupoUsuarios", !totalGrupoUsuarios);
  };

  useEffect(() => {
    updateGrupoUsuariosList();
  }, [totalGrupoUsuarios, itemsPerPage]);
  return (
    <>
      <div
      ...
      >
        ...
        <div className="d-flex justify-content-between align-items-center mx-5">
          <p>
            Mostrando {firstIndex} a {lastIndex} de {grupoUsuariosList.length}{" "}
            entradas
          </p>
          <PaginationTable
